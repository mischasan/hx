...
main(void)
{
    ...
    ok(hp != NULL, "hxopen lock_t.hx for update: %s",
       hp ? "ok" : strerror(errno));

    rc = hxput(hp, "a:1", 3);
    ok(rc == 0, "hxput(a:1): added record: (%d) %s", rc,
       rc < 0 ? hxerror(rc) : "");

    strcpy(buf, "a:?");
    rc = hxget(hp, buf, 0);
    ok(rc == 3, "hxget checks for key present: (%d): %s", rc,
       rc < 0 ? hxerror(rc) : "");
    ok(!strcmp(buf, "a:?"), "hxget(%s,0) leaves buffer value unchanged", buf);

    // Until hxhold(hp,*,0) is fixed, make it reject 0
    rc = hxhold(hp, buf, 0);
    ok(rc == HXERR_BAD_REQUEST, "PROTEM: reject hxhold(hp, *, 0): %s",
       hxerror(rc));
    rc = hxrel(hp);
    ...
    ret = hxhold(hp, buf, 3);
    ok(ret == 3, "hxhold acquires record=%.3s: (%d): %s", buf, ret,
       ret < 0 ? hxerror(ret) : "");
    ...
    buf[2] += 1;
    ret = hxput(hp, buf, 3);
    ok(ret > 0, "hxput updates val += 1; record=%.3s: (%d): %s", buf, ret,
       ret < 0 ? hxerror(ret) : "");
