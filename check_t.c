...
#   define  PGSIZE  256
    ok(!hxcreate("clean_t.hx", 0644, PGSIZE, "ch", 2),
	"hxcreate clean_t.hx");
    ...
    if (!ok(ret == HXERR_BAD_REQUEST, "hxfix rejects (uleng > 0, udata == NULL)"))
	fprintf(stderr, "hxfix returned %s\n", hxmode(ret));

    hxnext(hp, buf, sizeof(buf));

    ret = hxfix(hp, (FILE*)1, 0, NULL, 0);
    if (!ok(ret == HXERR_BAD_REQUEST, "hxfix(repair) during next is not allowed"))
	fprintf(stderr, "hxfix returned %s\n", hxmode(ret));
	...
	ok(ret == (HXRET)HX_UPDATE, "hxfix returned %s repairing zeroes at %d", hxmode(ret), pos);
	hxclose(hp);
	fclose(fp);
    }
