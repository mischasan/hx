...
_hxcheckbuf(HXLOCAL const*locp, HXBUF const*bufp)
{
    ...
    if (IS_MAP(hp, bufp->pgno)) {
        ...
        int recs = 0, is_tail = bufp->used && !bufp->next && !IS_HEAD(bufp->pgno);
        char *recp, *endp;
        ...
        FOR_EACH_REC(recp, bufp, endp) {
            ...
            if (!hx_test(hp, RECDATA(recp), size - sizeof(HXREC)))
                return bad_rec_test;
            if (head && !is_tail && head != _hxhead(locp, hx_hash(hp, RECDATA(recp)))) {
                DEBUG("bad head:%d", _hxhead(locp, hx_hash(hp, RECDATA(recp))));
                return bad_rec_hash;
            }
        }
...
_hxdebug(char const*func, int line, char const*fmt,...)
{
    ...
    fprintf(hxlog ? hxlog : stderr,
            hxtime ? "%s%s%s:%d%s\t%.6f\n": "%s%s%s:%d%s\n",
            msg, len < 64 ? "\t\t\t\t\t\t\t\t\t"  + len / 8 : "\t",
            func, line, hxproc ? hxproc : "",
            hxtime ? tick() - hxtime : 0.0);
    free(msg);
}
...
_hxheads(HXLOCAL *locp, HXBUF const*bufp, char *outstr)
{
    HXFILE *hp = locp->file;
    *outstr = 0;
    if (IS_MAP(hp, bufp->pgno)) return outstr;
    ...
    char *cp = outstr;
    while (nheads > 0) cp += sprintf(cp, "%d,", headv[--nheads]);
    if (cp > outstr) cp[-1] = 0;
    return outstr;
}
...
_hxprbuf(HXLOCAL const*locp, HXBUF const *bufp, FILE *fp)
{
    ...
    if (IS_MAP(hp, bufp->pgno)) {
        ...
        for (len = DATASIZE(hp); len > bufp->used && !bufp->data[len - 1];) --len;
...
void
_hxprloc(HXLOCAL const*locp, FILE *fp)
{
    HXFILE  *hp = locp->file;
    fprintf(fp, "{\tfile:{\tmlen:%ld mmap:%p tail:{pgno:%u used:%d recs:%d next:%d}\n",
            hp->mlen, hp->mmap, hp->tail.pgno, hp->tail.used, hp->tail.recs, hp->tail.next);
    fprintf(fp, "\t\thold:%d locked:%d lockv:[%d %d %d] lockpart:%d buffer:{...} head:%d ...}\n",
            hp->hold, hp->locked, hp->lockv[0], hp->lockv[1], hp->lockv[2],
            hp->lockpart, hp->head);
    fprintf(fp, "\tnpages:%d dpages:%d mask:%d hash:%08X head:%d mode:%d mylock:%d changed:%d\n",
            locp->npages, locp->dpages, locp->mask, locp->hash, locp->head, locp->mode, locp->mylock, locp->changed);
    fprintf(fp, "\tbuf:[...] ...\n");
    fprintf(fp, "}\n");
}

void
_hxprlox(HXFILE *hp, FILE *fp)
{
    static char who[99];
    fprintf(fp, "blocked by %s\n", _hxblockstr(hp, who));
}
