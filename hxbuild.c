...
typedef struct { HXREC *recp; PAGENO head; } REC;
typedef struct { off_t nbytes; int nrecs;  } PART;
...
static int cmprec(REC const *a, REC const *b)
{
    int cmp = (int)a->head - (int)b->head;
    return  cmp ? cmp
        : (int)RECHASH(a->recp) - (int)RECHASH(b->recp);
}

static int recdiff(HXFILE*hp, HXREC*a, HXREC*b)
{
...
hxbuild(HXFILE *hp, FILE *inp, int memlimit, double size)
{
    ...
    _hxresize(locp, 1 + _hxd2f((nbytes + MIN_INDEX_BYTES(nrecs))/(DATASIZE(hp) - nbytes/nrecs/2)));
    ...
    if (feof(inpf) && nbytes <= memlen) {
        ...
        for (i = 0; i < nparts; ++i) {
            ...
            DEBUG2("%d: %lld %d", i, partv[i].nbytes,
                    partv[i].nrecs);
    ...
    for (bufp->pgno = 0; bufp->pgno < lastmap; bufp->pgno = NEXT_MAP(hp, bufp->pgno)) {
        STAIN(bufp);
        _hxsave(locp, bufp);
    ...
    // HOLD_FILE keeps other hx calls from locking/unlocking.
    HOLD_FILE(hp);
...
_flush(HXLOCAL *locp, int vbufsize, int i, off_t nbytes)
{
    ...
    DEBUG2("part:%3d %3d len:%d %d", i, (int)(nbytes/vbufsize),
            len, nbytes % vbufsize);
    if (0 > lseek(fileno(locp->fp[2]),
                    i * locp->memsize + nbytes - len, 0))
        LEAVE(locp, HXERR_LSEEK);

    if (0 > write(fileno(locp->fp[2]),
                    locp->mem + i * vbufsize, len))
        LEAVE(locp, HXERR_WRITE);
}

static void
_parse(HXLOCAL *locp, char *inpbuf, int len,
        HXREC *rp, int recsize)
{
    if (inpbuf[--len] != '\n')
...
_split(HXLOCAL*locp, PART*partv, int nparts, int fd, FILE*inpf)
{
    ...
    int         vbufsize = locp->memsize / DISK_PGSIZE
                                / nparts * DISK_PGSIZE;
