...
hxput(HXFILE * hp, char const*recp, int leng)
{
    ...
    _hxload(locp, currp,
            SCANNING(hp) &&(leng || IS_HEAD(hp->buffer.pgno))
    ...
    while (1) {
        ...
        if (pos >= 0) {
            ...
            if (!newsize) {     // hxdel or remove after inserted previously.
                ...
                if (delta) {
                    memmove(oldp + newsize, oldp + oldsize, currp->used - pos - oldsize);
                    currp->used += delta;
        ...
        if (skip) LINK(prevp, nextpg); else SWAP(prevp, currp);
