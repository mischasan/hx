...
_hxislocked(HXLOCAL const *locp, PAGENO pg)
{
    ...
    return !pg ? hp->locked & LOCKED_ROOT
        ...
        : hp->locked & LOCKED_BODY || *findlock(hp, pg);
}
...
_hxlock(HXLOCAL * locp, PAGENO pgno, COUNT count)
{
    ...
    if (!pgno && !count) {
        if (hp->locked & LOCKED_ROOT)
            pgno = 1;
        if (hp->locked & LOCKED_BODY)
            count = 1;
        if (pgno && count)
            return;
    }

    if (pgno == 0 && count == 1 && hp->locked & LOCKED_ROOT)
        return;
    if (pgno == 1 && count == 0 && hp->locked & LOCKED_BODY)
        return;
    if (pgno >= locp->npages && hp->locked & LOCKED_BEYOND)
        return;
    if (pgno && count && hp->locked & LOCKED_BODY)
        return;

    // Skip pages already locked
    if (count) {
        while (count && _hxislocked(locp, pgno))
            ++pgno, --count;
        if (!count)
            return;
    }
    ...
    if (hxdebug && count && pgno < locp->npages && IS_HEAD(pgno)) {
        PAGENO *pp = hp->lockv;

        while (*pp && (!IS_HEAD(*pp) || pgno <= *pp))
            ++pp;
        if (*pp)
            DEBUG("pgno=%u count=%d after %u", pgno, count, *pp);
    }
...
_hxlockset(HXLOCAL * locp, LOCKPART part)
{
    ...
    PAGENO  pgv[2 * HXPGRATE] = { }
    , *pp, oldsize = 0;

    while (1) {
        _hxsize(locp);
        _hxpoint(locp);         // calculate locp->head
        if (oldsize == locp->npages)
            break;
        oldsize = locp->npages;
        for (pp = pgv; *pp; ++pp)
            _hxunlock(locp, *pp, 1);
...
_lock(HXLOCAL * locp, off_t pos, short mode, off_t len)
{
    ...
    if (errno && errno != EINTR) {
        ...
        DEBUG
            ("LOCK ERROR: %lu(%lu) mode=%s errno %d='%s' locks %s blocked by "
             "%d(%d) pid:%d", (PAGENO) (pos / hp->pgsize),
             (PAGENO) (len / hp->pgsize),
             (unsigned)mode <= F_UNLCK ? modename[mode] : "???", save,
             strerror(save), lockstr(hp, str),
