...
#   define  _ "....:...."
    char const val[] =
        "k.......\0" _"1" _"2" _"3" _"4" _"5" _"6" _"7" _"8" _"9" _"0" _"A"
        _"B" _"C" _"D" _"E";
    char    buf[300];
...
#   undef   _
    ...
    system("unset HXDEBUG; set -x; if $hx/chx -v check conc_t.hx; then $hx/chx info conc_t.hx; $hx/chx stat conc_t.hx; fi >&2");
    char    who[99];
    ...
    if (!fork()) {
        ...
        for (i = 0; rc >= 0 && i < nrecs * niters - 1; ++i) {
            ...
            if (hxdebug)
                fprintf(stderr, "    hxdel(%s)...\n", buf);
            rc = hxdel(hp, buf);
            if (hxdebug)
                fprintf(stderr, "    hxdel(%s): %d\n", buf, rc);
            if (rc > 0)
                ++nops;
        }
        if (rc < 0)
            fprintf(stderr, "failed on hxdel(%s)\n", buf);
    ...
    for (j = 0; j < niters; ++j) {
        ...
        if (rc < 0) {
            fprintf(stderr, "failed on hxput(%s): %s\n", buf, hxerror(rc));
            break;
        }
    }
    ...
    system
        ("unset HXDEBUG; set -x; if chx -dv check conc_t.hx; then chx info conc_t.hx; chx stat conc_t.hx; fi >&2");
