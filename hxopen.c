...
static int _hxlib(HXFILE*, char const*path, char const *name,
                    char **pathp);
//--------------|---------------------------------------------
HXFILE *
hxopen(char const*name, HXMODE mode)
{
    ...
    if ((vp = getenv("HXDEBUG"))) hxdebug = atoi(vp);
    if (!hxproc || !*hxproc)      hxproc = getenv("HXPROC");
    ...
    if (fix || (OK_VERSION(version) && udata && mlen > 0 && mlen % pgsize == 0)) {
	...
	if (udata) {
	    hp->udata[hp->uleng] = 0;
	    if (!(hp->mode & HX_STATIC)) hxlib(hp, hp->udata, 0);
	}
...
static int
_hxlib(HXFILE *hp, char const *ep, char const *hxreclib,
        char **pathp)
{
    int		len;
    void        *dlp = 0;
