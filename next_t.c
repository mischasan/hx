...
main(void)
{
    ...
    for (mode = 0; mode <= HX_MMAP; mode += HX_MMAP) {  //mode <= HX_MMAP
        ...
        while (--recp >= recv && 0 <= (rc = hxput(hp, *recp, strlen(*recp)))) {
            if (hxdebug)
                fprintf(stderr, "# >%s\n", *recp);
        }
        ...
        ok(actual == nrecs, "first scan returned %d/%d records", actual,
           nrecs);
        ...
        ok(rc == HXERR_BAD_REQUEST,
           "update of different key while scanning is rejected: %s",
           hxerror(rc));

        rc = hxput(hp, buf, len + 1);
        ok(rc == len, "small increase in record size allowed: %s",
           hxerror(rc));

        rc = hxput(hp, buf, len - 1);
        ok(rc > len - 1, "update that shortens record accepted: %s",
           hxerror(rc));
        ...
        rc = hxdel(hp, buf);
        ok(rc == len - 1, "delete of %.*s accepted: %s", len, buf,
           hxerror(rc));
        ...
        while ((len = hxnext(hp, buf, sizeof buf)) > 0) {
            if (!scanned++) {
                errno = 0;
                ok(test_locked(hp) == 1,
                   "first hxnext leaves file locked: %s", strerr());
            }

            if (hxdebug)
                fprintf(stderr, "# hxnext: %.*s\n", len, buf);
            if (buf[0] < 'm' || !memcmp(buf, "yo", 2)) {
                rc = hxdel(hp, buf);
                if (rc > 0)
                    ++deleted;
                if (hxdebug)
                    fprintf(stderr, "# hxdel(%.*s): %s\n", len, buf,
                            hxerror(rc));
            }
        }

        errno = 0;
        ok(test_locked(hp) == 0, "final hxnext leaves file unlocked: %s",
           strerr());
        ok(len == 0, "scan with deletes completed: %s", hxerror(len));
        ok(scanned == nrecs, "%d/%d records scanned for deletions", scanned,
           nrecs);
        ok(deleted == deletable, "%d/%d deletable records deleted", deleted,
           deletable);
