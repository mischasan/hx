// Copyright(c)1992 Mischa Sandberg, Vancouver, Canada.
...
typedef struct { HXHASH hash; COUNT leng; } PACKED HXREC;
...
static inline unsigned RECLENG(void const *rp)
{ return LDUS(sizeof(PAGENO) + (char const*)rp); }

static inline unsigned RECSIZE(void const *rp)
{ return sizeof(HXREC) + RECLENG(rp); }

static inline char const* RECDATA(void const *rp)
{ return sizeof(HXREC) + (char const*)rp; }

typedef struct { PAGENO pgno; COUNT used, recs; } PGINFO;
...
#   define  MAXLOCKS (HX_MAX_CHAIN + 4)
...
static inline PAGENO _hxmask(PAGENO x)
{
    if (!x--) return 0;
...
void    _hxprloc(HXLOCAL const*, FILE*) regargs;
void    _hxprlox(HXFILE*, FILE*) regargs;
...
//--------------|---------------------------------------------
// HIND_BASE is the LAST element in hind[].
static inline void     BUFLINK(HXLOCAL*, HXBUF*, PAGENO) regargs;
static inline int      BUFNUM(HXLOCAL const*, HXBUF const*) regargs;
static inline unsigned DATASIZE(HXFILE const*) regargs;
static inline int      FILE_HELD(HXFILE const*) regargs;
static inline unsigned FITS(HXFILE const*, HXBUF const*, int dused, int drecs) regargs;
static inline int      HEAD_HELD(HXLOCAL const*) regargs;
static inline COUNT*   HIND_BASE(HXFILE const*, HXBUF*) regargs;
static inline COUNT const*HIND_BASE_C(HXFILE const*, HXBUF const*) regargs;
static inline COUNT    HIND_POS(HXBUF const*, HXHASH) regargs;
static inline int      HIND_SIZE(HXFILE const*, HXBUF const*) regargs;
static inline void     HOLD_FILE(HXFILE*) regargs;
static inline void     HOLD_HEAD(HXLOCAL*) regargs;
static inline int      IMIN(int a, int b) regargs;
static inline int      IS_HEAD(PAGENO) regargs;
static inline int      IS_MAP(HXFILE const*, PAGENO) regargs;
static inline int      IS_MMAP(HXFILE const*) regargs;
static inline void     MAP_BUF(HXFILE const*, HXBUF*) regargs;
static inline int      MAX_RECS(HXFILE const*) regargs;
static inline int      MIN_INDEX_BYTES(int nrecs) regargs;
static inline PAGENO   NEXT_MAP(HXFILE const*, PAGENO) regargs;
static inline int      OK_VERSION(COUNT version) regargs;
static inline HXHASH   REV_HASH(HXHASH) regargs;
static inline unsigned ROOT_SIZE(HXFILE const*) regargs;
static inline int      SCANNING(HXFILE const*) regargs;
static inline PAGENO   SPLIT_PAGE(HXLOCAL const*) regargs;
static inline unsigned _FITS(HXFILE const*, int sused, int srecs, int dused, int drecs) regargs;

static inline char*    _hxcurrec(HXFILE const*) regargs;
static inline PAGENO   _hxd2f(PAGENO) regargs;
static inline PAGENO   _hxf2d(PAGENO) regargs;

static inline char *
_hxcurrec(HXFILE const *hp)
...
{
    return mappg + 8 * HXPGRATE * 
            (DATASIZE(hp) - (mappg ? 0 : -hp->uleng));
}
