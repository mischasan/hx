...
int
hx_load(HXFILE const *hp, char *recp, int recsize,
        char const *buf)
{
    if (!hp->load)
        ...
        char *buf, int bufsize)
{
    return !hp->save ? HXERR_BAD_REQUEST
                     : hp->save(recp, reclen, buf, bufsize,
                                hp->udata, hp->uleng);
}
...
{
    return hp->test
            ? hp->test(recp, reclen, hp->udata, hp->uleng)
            : HXERR_BAD_REQUEST;
}

int hxfileno(HXFILE const *hp)
{ return hp ? hp->fileno : -1; }

int
hxinfo(HXFILE const *hp, char *udata, int usize)
{
    if (!hp) return HXERR_BAD_REQUEST;
...
_hxenter(HXLOCAL *locp, HXFILE *hp, char const*recp, int nbufs)
{
    ...
    errno = 0;
    if (recp) locp->hash = hx_hash(hp, recp);
...
int
_hxfind(HXLOCAL *locp, HXBUF const*bufp, HXHASH rechash, char const*recdata, int *hindp)
{
    HXFILE const *hp = locp->file;
    COUNT const *hind = HIND_BASE_C(hp, bufp);
    int hsize = HIND_SIZE(hp, bufp);
    unsigned mask = _hxmask(hsize);
    int i = rechash & mask;
    if (i >= hsize) i &= mask >> 1;
...
_hxindexed(HXLOCAL*locp, HXBUF const*bufp)
{
    ...
    while (i < hsize && hpos > 0) hpos -= !test[i++];
    if (i == hsize)
        return 0;
    ...
    for (i = 0; i < hsize; ++i) if (tend[-i]) return 0;
...
_hxpginfo(HXLOCAL*locp, PAGENO pg)
{
    ...
    return (PGINFO){ LDUL(&info.pgno), LDUS(&info.used), LDUS(&info.recs) };
}
...
#           else
        ...
        DEBUG("mmap %p[%d] -> %p[%d]", hp->mmap, hp->mlen, mp, mlen);
        if (!mp || mp == MAP_FAILED)
            LEAVE(locp, HXERR_MMAP);
        ...
        while (--bufp >= locp->buf) if (bufp->page) MAP_BUF(hp, bufp);
    }
}
...
_hxsize(HXLOCAL *locp)
{
    ...
    if (size % hp->pgsize || (size /= hp->pgsize) < 2)
        LEAVE(locp, HXERR_BAD_FILE);
    if (locp->npages && locp->npages != size)
        DEBUG2("npages changes: %d >> %d", locp->npages, size);
    locp->npages = size;
