...
static int mysave(char const*recp __unused, int reclen __unused, char const*str __unused, int bufsize __unused, char const*udata __unused, int uleng __unused) { return 3; }
...
main(void)
{
    ...
    ok(!hxopen("basic_t.hx", HX_READ), "PGSIZE too small: %s",
       strerror(errno));
    hxclose(hxopen("basic_t.hx", HX_REPAIR));
    ...
    lseek(fd, 0, 0);
    write(fd, &root, sizeof root);
    ok(!hxopen("basic_t.hx", HX_READ), "PGSIZE not a power of 2: %s",
       strerror(errno));
    hxclose(hxopen("basic_t.hx", HX_REPAIR));
    ...
    ok(hp =
       hxopen("basic_t.hx", HX_REPAIR), "REPAIR accepts PGRATE too low: %s",
       strerror(errno));
    hxclose(hp);
    ...
    lseek(fd, 0, 0);
    write(fd, &root, sizeof root);
    ok(!hxopen("basic_t.hx", HX_READ), "PGRATE too high: %s",
       strerror(errno));
    ok(hp =
       hxopen("basic_t.hx", HX_REPAIR), "REPAIR accepts PGRATE too high");
    hxclose(hp);

    root = (HXROOT) {
    128, 4, 1, 0};              // filesize < uleng
    lseek(fd, 0, 0);
    write(fd, &root, sizeof root);
    ok(!hxopen("basic_t.hx", HX_READ), "filesize < uleng: %s",
       strerror(errno));
    ok(hp =
       hxopen("basic_t.hx", HX_REPAIR), "REPAIR accepts filesize < uleng");
    hxclose(hp);

    root = (HXROOT) {
    32, 4, 27, 0};              // uleng >= pgsize-6
    lseek(fd, 0, 0);
    write(fd, &root, 32);
    ok(!hxopen("basic_t.hx", HX_READ), "uleng >= pgsize-6");
    ok(hp =
       hxopen("basic_t.hx", HX_REPAIR), "REPAIR accepts uleng >= pgsize-6");
    hxclose(hp);

    close(fd);
    ok(!hxopen("basic_t.hx", -1), "hxopen rejects bad MODE arg: %s",
       strerror(errno));
    hxclose(NULL);
    ...
    ok(hp =
       hxopen("basic_t.hx", 0),
       "open hxfile with version matching hxversion: %s", strerror(errno));
    hxclose(hp);

    hxversion += 0x0001;
    ok(hp =
       hxopen("basic_t.hx", 0),
       "open hxfile with version same major version and lower minor version: %s",
       strerror(errno));
    hxclose(hp);
    ...
    if (!ok
        (!hp, "hxopen rejects hxfile not matching library major version: %s",
         strerror(errno)))
        hxclose(hp);
    ...
    ok(hp =
       hxopen("basic_t.hx", 0), "open hxfile with 32K pagesize: %s",
       strerror(errno));
    hxclose(hp);
    ...
    ok(rc == HXOKAY, "create hxfile with only 1 map byte in root page: %s",
       hxerror(rc));
    ...
    ok(!memcmp(udata, "ch\0de?????", 10),
       "hxinfo fills small buffer as specified");
    ...
    ok((HXRET) HX_UPDATE == (rc = hxfix(hp, 0, 0, 0, 0)),
       "before puts hxcheck says: %s", hxmode(rc));
    for (buf[0] = 'A'; buf[0] < 'A' + HX_MAX_CHAIN - 2; ++buf[0]) {
        rc = hxput(hp, buf, len = 10 + buf[0] % 3);
        ok(rc == 0, "hxput('%c...',%d) returned %s (%d)", buf[0], len,
           hxerror(rc), rc);
        ok((HXRET) HX_UPDATE == (rc = hxfix(hp, 0, 0, 0, 0)),
           "... and hxcheck says: %s", hxmode(rc));
    }

    for (buf[0] = 'A'; buf[0] < 'A' + HX_MAX_CHAIN - 2; ++buf[0]) {
        rc = hxget(hp, buf, 14);
        ok(rc == 10 + buf[0] % 3, "hxget('%c...') returned %s (%d)", buf[0],
           hxerror(rc), rc);
    }

    ok(HXERR_BAD_REQUEST == hxput(hp, NULL, 0), "hxput rejects NULL record");
    ok(HXERR_BAD_REQUEST == hxput(hp, buf, -1),
       "hxput rejects record length < 0");
    buf[0] = 1;
    buf[1] = 0;
    ...
    rc = hxfix(hp, 0, 0, 0, 0);
    ok(rc == (HXRET) HX_UPDATE,
       "check works on hxfile with an overflow map page: %s", hxmode(rc));
    buf[1] = '-';
    buf[2] = 0;
    ...
    if (!ok(rc == HXOKAY, "hxpack works on hxfile with an overflow map page"))
    ...
    ok(rc == HXOKAY, "create hxfile for recursive GrowFile test: %s",
       hxerror(rc));
    hp = hxopen("basic_t.hx", HX_UPDATE | HX_RECOVER);
    ...
    for (buf[0] = 'X', buf[1] = 'A', buf[2] = '[';
        ...
        hxput(hp, buf, buflen);
        rc = hxfix(hp, 0, 0, 0, 0);
        ok(rc == (HXRET) HX_UPDATE, "file okay after inserting %.2s: %s", buf,
           hxmode(rc));
    }
    ...
    rc = hxstat(NULL, NULL);
    ok(rc == HXERR_BAD_REQUEST, "hxstat returns %s for NULL HXSTAT* arg",
       hxerror(rc));
    ...
    rc = hxget(NULL, buf, 1);
    ok(rc == HXERR_BAD_REQUEST, "hxget rejects NULL HXFILE* arg: %s",
       hxerror(rc));

    rc = hxnext(NULL, buf, 1);
    ok(rc == HXERR_BAD_REQUEST, "hxnext rejects NULL HXFILE* arg: %s",
       hxerror(rc));

    rc = hxcreate("basic_t.hx", 0755, 2049, "test", 5);
    ok(rc == HXERR_BAD_REQUEST, "hxcreate rejects bad pgsize: %s",
       hxerror(rc));

    rc = hxcreate("basic_t.hx", 0755, 256, "ch", 251);
    ok(rc == HXERR_BAD_REQUEST, "hxcreate rejects bad uleng: %s",
       hxerror(rc));

    system("PS4='# '; set -x; touch bad_t.hx; chmod 000 bad_t.hx");
    rc = hxcreate("bad_t.hx", 0755, 256, "ch", 2);
    ok(rc == HXERR_CREATE,
       "hxcreate reports failure for unwritable target file: %s",
       hxerror(rc));

    rc = hxcreate("basic_t.hx", 0755, 0, "ch", 2);
    ok(rc == HXOKAY, "created basic_t.hx with default pgsize: %s",
       hxerror(rc));

    ok(!stat("basic_t.hx", &sb), "file created: %s", strerror(errno));
    // blksize: FreeBSD => uint32_t; Linux => long int.
    ok(sb.st_size == 2 * sb.st_blksize, "default blocksize (%d) used",
       (int)sb.st_blksize);
    ...
    len = hxget(hp, buf, 1);
    ok(len == 0, "hxget on empty file returns no record: %s (%d)",
       hxerror(rc), rc);
    rc = hxput(hp, buf, 4);
    ok(rc == HXERR_BAD_REQUEST,
       "hxput rejected update of read-only HXFILE: %s", hxerror(rc));
    hxdebug = 0;
    ...
    for (mode = HX_UPDATE; mode <= HX_UPDATE + HX_FSYNC; mode += HX_MMAP) {
        ...
        ok(rc == 34, "hxput updates record that forces file to grow: %s",
           hxerror(rc));
        ...
        ok(rc == HXERR_BAD_REQUEST,
           "hxput rejected record too large for file: %s", hxerror(rc));

        rc = hxdel(hp, buf);
        ok(rc == hxmaxrec(hp), "first hxdel reported record deleted: %s (%d)",
           hxerror(rc), rc);

        buf[1] = ':';
        rc = hxdel(hp, buf);
        ok(rc == 4, "second hxdel reported record deleted: %s (%d)",
           hxerror(rc), rc);

        diag("set hxdebug=3 strictly for coverage");
        hxdebug = 3;
        rc = hxdel(hp, buf);
        ok(rc == 0, "redundant hxdel reported no record to delete: %s (%d)",
           hxerror(rc), rc);
    ...
    system("chmod -x $hx/hx_badh.so");
    hp = hxopen("bad_t.hx", HX_READ);
    ok(!hp, "cannot open file with unexecutable rectype: %s", strerror(errno));
    system("chmod +x $hx/hx_badh.so");
    system("mv $hx/hx_badh.so $hx/hx_badh.so.bak");
    ok(!hp, "cannot open file with unexecutable rectype: %s", strerror(errno));
    system("mv -f $hx/hx_badh.so.bak $hx/hx_badh.so");
    ...
    hxbind(hp, (HX_DIFF_FN) mydiff, (HX_HASH_FN) myhash,
               (HX_LOAD_FN) myload, (HX_SAVE_FN) mysave, mytest);
