...
static int
memspan(char const *buf, char ch, int len)
{
...
#   define  OK(...) ++test[_ok(fp, __VA_ARGS__)]
    ...
    OK(len1 > 0 &&
       len1 <= maxrec, "hx_load(,,0,) returns: %d in 1..%d for '%s'", len1,
       maxrec, str);
    OK(memspan(recspc, fill, reclim),
       "hx_load(,rec,0,) leaves rec unchanged");

    int     len2 = hx_load(hp, rec, len1, str);

    OK(len2 == len1 && memspan(recspc, fill, 5)
       && memspan(rec + len1, fill, reclim - 5 - len1), "hx_load(,,%d,): %d",
       len1, len2);
    ...
    for (i = 1; i < len1; ++i) {
        ...
        OK(len2 == len1 && memspan(recspc, fill, 5) && !memcmp(rec, buf, i)
           && memspan(rec + i, fill, reclim - 5 - i), "hx_load(,,%d,): %d", i,
           len2);
    }
    ...
    OK(memspan(bufspc, fill, buflim),
       "hx_save(,,,,0) leaves target unchanged");
    ...
    OK(len3 == len2 && memspan(buf, fill, 7) &&
       memspan(buf, fill, buflim - 7 - len2)
